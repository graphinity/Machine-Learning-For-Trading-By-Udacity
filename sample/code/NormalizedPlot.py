...
def get_data(symbols, dates):
    ...
    for symbol in symbols:
        ...
        df = df.join(df_temp)
        if symbol == 'Google':  # drop dates SPY did not trade
            df = df.dropna(subset=["Google"])

    return df

def normalize_plot(df):
...
def test_run():
    ...
    symbols = ['Google', 'Apple']  # SPY will be added in get_data()
    
    # Get stock data
    df = get_data(symbols, dates)

    # Slice and plot
    plot_selected(df, ['Google', 'Apple'], '2010-03-01', '2010-04-01')
